...
export default class JumpingGame extends Phaser.Scene {
  ...
  create() {
   ...
   const zone = [];
    ...
    this.stars = this.physics.add.staticGroup({
      key: 'star',
      repeat: 11,
      setXY: { x: 12, y: 100, stepX: 45 }
    });

    this.stars.children.iterate((child) => {
      this.moveObjects(child, 'down')
    })
  ...
  moveObjects(child, direction){
    if (direction === 'down'){
      const moveDown = setInterval(()=>{
        child.y += 1;
        if (child.y >= 800) {
          clearInterval(moveDown);
          this.moveObjects(child, 'up')
        }
      },10)
    }
    if (direction === 'up'){
